...
- `reality_settings` (Optional, Block) - Reality settings.
  ...
  - `min_client_ver` (Optional, String) - Minimum client Xray version the REALITY server accepts, as `major.minor.patch` (e.g. `26.3.27`). Version-dependent default: with xray-core 26.7.x (3x-ui up to v3.7.x) an empty value is **not** the same as disabling the gate — the core substitutes `26.3.27`, rejecting clients that report an older or absent version (e.g. sing-box); with xray-core 26.9.8+ (3x-ui v3.8.x) an empty value means no minimum. Set `0.0.0` to remove the lower bound on every supported panel version — that is the canonical spelling, and `0` / `0.0` are zero-filled to the same version. One to three components, each in `0-255`; an empty string is rejected. See the note on widening a bound below.
