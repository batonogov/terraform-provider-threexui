...
Six resources — `panel_general`, `panel_security`, `panel_telegram`,
`panel_email`, `panel_discord`, `panel_subscription` — share
`resource_settings_tabs.go`.
...
- Changing a restart-triggering key triggers a provider-initiated restart
  ...
  - `panel_telegram` / `panel_email`: the notifier cron registration, read once in
    ...
    - `panel_discord` (v3.8.0+) is asymmetric to Telegram: the notify cron AND the
      gateway hot-reload in-process on settings update (`controller.SetReloadDiscordFunc`,
      `3x-ui-3.8.5/internal/web/controller/setting.go:182-189` → `web.go:693-719`), so
      `discordRunTime`/`discordBotToken`/`discordChannelId` do **not** restart — but
      `discordBotEnable` does (the alarm samplers read it once at boot, `web.go:478-482`,
      and the reload func never re-checks alarm registration), and
      `discordEnabledEvents`/`discordCpu`/`discordMemory` follow the same derived rules
      as tg/smtp. On v3.8.x `/setting/all` returns `AllSettingView` with every secret
      blanked (`GetAllSettingView`, `setting.go:317-340`) — the provider replays the
      last configured `discordBotToken` from state/cache, and a blank token in a POST
      means "keep the stored one" (`preserveRedactedSecrets`, `setting.go:1621-1627`).
  ...
  would bounce it on every apply — 2 of the 10 versions in `compat-versions.json`
...
| `panel_discord` | `discord_bot_token` | `discord_bot_token_wo` | `discord_bot_token_wo_version` |
...
| Protocol names are version-specific | `hysteria2` is a distinct protocol only on v3.0.2/v3.1.0; from v3.2.0 use `protocol = "hysteria"` with `streamSettings.version = 2`. `tunnel` requires ≥ v3.2.0, `tun` requires ≥ v3.2.7, `mtproto` requires ≥ v3.3.0, and `tuic` requires ≥ v3.8.0 (TUIC v5 sidecar; settings are `{"server": {...}, "clients": [...]}` with **snake_case** wire keys, nested like AmneziaWG — see `inbound_tuic_schema.go`. The panel generates nothing: certificate/private_key inline PEM are operator-supplied, other server fields default at read time. Clients are owned by the inbound (`protocolOwnsClients`), require id+password+email on every save, and their flatten normalises absent keys to zero values because the panel serialises a TUIC client differently on the create vs update read-back — null-vs-"" there fails the apply with an inconsistent-result error) |
...
| v3.8.x AllSetting + protocol delta | 3x-ui v3.8.0/v3.8.5 **added 42 `AllSetting` fields** (147 total, 0 removed): a Discord notification bot (10 `discord*` keys incl. sensitive `discordBotToken`, mirroring the Telegram bot — managed by the `panel_discord` resource), Happ client customization (`happLinkEnable` + 22 `subHapp*`), `subProfileMode` (None/Built-in/Custom — the built-in profile page is now opt-in and subscription responses stop advertising it, #6538; **v3.8.5-only** — the key does not exist on v3.8.0, where it reads back null), `subInfoNodeEnable`/`subCalendarExpireInclusive`/`subExpiredTemplate`/`subTrafficDepletedTemplate`, `subJsonRoutingRules`/`subJsonDns`, and `realityScanCandidates`. All are provider-managed: discord by `panel_discord`, the Happ/profile-page/JSON-subscription group by `panel_subscription` (restart keys: `subProfileMode`, `subJsonRoutingRules`, `subJsonDns` and all 22 `subHapp*` — initRouter-frozen, `sub.go:153-256`; per-request and quiet: `subCalendarExpireInclusive`, `subInfoNodeEnable`, the expired/depleted templates, `happLinkEnable`), and `realityScanCandidates` by `panel_general`. `Inbound`/`Client` structs are unchanged. A new `tuic` protocol (TUIC v5, terminated by a bundled `tuic-server` sidecar, not xray-core) is skipped in the protocol drift gates until implemented. xray-core bumped to **v26.9.9**, and one-time startup migrations **rewrite stored xray templates**: outbound `proxySettings.tag` → `streamSettings.sockopt.dialerProxy` (the old key is now refused by the loader), freedom `addressPortStrategy` dropped, freedom `domainStrategy` deprecated in favour of `sockopt.domainStrategy`, DNS outbound legacy `nonIPQuery`/`blockTypes` → rules, and numeric `"qType": 0` → `"0"` (v3.8.5) — provider-managed `xray_outbounds`/`xray_dns` state may show a one-time diff after the panel upgrades to v3.8.x. Saving or enabling an inbound whose ports collide (incl. AmneziaWG peer forwards) is now **refused** with the owning row named (v3.8.5) instead of silently taking the core down; `restartXrayOnClientDisable` now also fires on manual client disable/delete (more `"Unknown"` xray-version windows in acc tests); a wrong or disabled Bearer API token answers 401 instead of 404 (a missing `Authorization` header is still masked as 404). **TLS inbounds need certificate material:** `validateInboundTLSCertificates` (`inbound.go:686`) refuses to save any `security = "tls"` inbound whose `tlsSettings.certificates[]` carries no server cert+key (file path or inline; only presence of the path is checked; `usage = "verify"` rows exempt; update is grandfathered only when the *old* stream settings also fail — `inbound.go:1729`). The provider models `tls_settings.certificates` as a nested **attribute** (`ListNestedAttribute` — blocks cannot nest inside `SingleNestedBlock`): file-backed (`certificate_file`/`key_file`) or inline PEM (`certificate`/`key`, key is Sensitive). It is `Optional + Computed`: omitting keeps existing entries, `certificates = []` clears them explicitly. Acc tests generate a self-signed cert (`testAccSelfSignedCertPEM`) — file paths are only presence-checked by the panel, but a path that doesn't exist on the panel host breaks the v3.8.5 config probe, so CI sticks to inline PEM; hysteria fixtures still omit `security = "tls"` (they test protocol coverage, not TLS). **State-address quirk:** because `certificates` is an attribute under single-nested blocks, flatmap addresses drop the block indices — `stream_settings.tls_settings.certificates.0.certificate.0` (NOT `stream_settings.0.tls_settings.0.…`) in TestCheckResourceAttr |
...
- Version-aware skipping: `requireMinVersion(t, "vX.Y.Z")` for features added in a
  ...
  upstream (e.g. `hysteria2`, dropped in v3.2.0). Currently supported: **v3.3.x**,
  **v3.4.x**, **v3.5.x**, **v3.6.x**, **v3.7.x**, **v3.8.x** (up to v3.8.5).
  `requireBelowVersion(t, "v3.2.0")` gates (legacy `dokodemo-door`, `hysteria2`)
  no longer fire in CI now that v3.2.x is out of the matrix — they only run when
